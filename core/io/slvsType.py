...
class slvsTypeSettings(QDialog, Ui_Dialog):
    ...
    def on_LinkHeader_textEdited(self, p0):
        self.LinkPreview.setText(self.nameRule('Line', self.LinkRule.currentIndex(), p0).format(0, 75.02))
    @pyqtSlot(str)
    def on_ChainHeader_textEdited(self, p0):
        self.ChainPreview.setText(self.nameRule('Chain', self.ChainRule.currentIndex(), p0).format(0, '20_10.05_12.4'))
    @pyqtSlot(str)
    def on_AssemblyHeader_textEdited(self, p0):
        self.AssemblyPreview.setText(p0+'0' if p0!='' else 'Assembly')
    ...
    def save(self):
        if self.hasCreateFolder.checkState():
            ...
            self.folderPath = QDir(self.Environment_variables)
        ...
        if self.hasAssembly.checkState():
            self.write(self.AssemblyPreview.text()+'.slvs', slvsAssembly(self.Point, self.Line, self.Chain))
    def write(self, fileName, content):
        filePath = QFileInfo(self.folderPath, fileName).absoluteFilePath()
        with open(filePath, 'w', encoding='iso-8859-15', newline="") as f:
            f.write(content)
    ...
    def on_ChainType_currentIndexChanged(self, index):
        ...
        if index==1:
            self.ChainImage.setPixmap(QPixmap(":/icons/preview/Chain_frame.png"))
