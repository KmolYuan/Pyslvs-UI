...
class Lists:
    ...
    def deletePointTable(self, Point, Line, Chain, Shaft, Slider, Rod, pos):
        ...
        self.FileState.push(deleteTableCommand(Point, pos))
        self.FileState.endMacro()
    ...
    def replacePoint(self, Line, Chain, Shaft, Slider, Rod, pos, Judgment, toNew):
        for e in self.LineList:
            ...
            if Judgment(e.start, pos):
                self.FileState.push(changePointNumCommand(Line, toNew(e.start), row, 1))
            if Judgment(e.end, pos):
                self.FileState.push(changePointNumCommand(Line, toNew(e.end), row, 2))
        for e in self.ChainList:
            row = self.ChainList.index(e)
            if Judgment(e.p1, pos):
                self.FileState.push(changePointNumCommand(Chain, toNew(e.p1), row, 1))
            if Judgment(e.p2, pos):
                self.FileState.push(changePointNumCommand(Chain, toNew(e.p2), row, 2))
            if Judgment(e.p3, pos):
                self.FileState.push(changePointNumCommand(Chain, toNew(e.p3), row, 3))
        for e in self.ShaftList:
            row = self.ShaftList.index(e)
            if Judgment(e.cen, pos):
                self.FileState.push(changePointNumCommand(Shaft, toNew(e.cen), row, 1))
            if Judgment(e.ref, pos):
                self.FileState.push(changePointNumCommand(Shaft, toNew(e.ref), row, 2))
        for e in self.SliderList:
            row = self.SliderList.index(e)
            if Judgment(e.cen, pos):
                self.FileState.push(changePointNumCommand(Slider, toNew(e.cen), row, 1))
            if Judgment(e.start, pos):
                self.FileState.push(changePointNumCommand(Slider, toNew(e.start), row, 2))
            if Judgment(e.end, pos):
                self.FileState.push(changePointNumCommand(Slider, toNew(e.end), row, 3))
        for e in self.RodList:
            row = self.RodList.index(e)
            if Judgment(e.cen, pos):
                self.FileState.push(changePointNumCommand(Rod, toNew(e.cen), row, 1))
            if Judgment(e.start, pos):
                self.FileState.push(changePointNumCommand(Rod, toNew(e.start), row, 2))
            if Judgment(e.end, pos):
                self.FileState.push(changePointNumCommand(Rod, toNew(e.end), row, 3))
    ...
    def deleteParameterTable(self, table, Point, Line, Chain, pos):
        ...
        self.FileState.push(deleteTableCommand(table, table.currentRow(), False))
        self.FileState.endMacro()
    
    def replaceDigit(self, table, pos, *column):
        for row in range(table.rowCount()):
            for k in column:
                if table.item(row, k).text()=='n{}'.format(pos):
                    self.FileState.push(changePointNumCommand(table, self.ParameterList[pos], row, k, 'n'))
    
    def batchMove(self, table, x, y, Points):
        self.FileState.beginMacro("Batch move {{{}}}".format(', '.join(['Point{}'.format(i) for i in Points])))
        for row in Points:
            self.FileState.push(editTableCommand(table, row,
    ...
    def m(self, p1, p2):
        ...
        try:
            ...
            angle = self.CosineTheoremAngleE(y, x, d)
