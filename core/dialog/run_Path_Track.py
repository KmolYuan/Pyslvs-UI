...
class Path_Track_show(QDialog, PathTrack_Dialog):
    ...
    def start(self):
        self.work = WorkerThread(self.Point, self.Link, self.Chain, self.Shaft, self.Slider, self.Rod, self.warning)
        ...
        if not self.Run_list.count()==0:
            limit = 0
            for e in self.work.Shaft:
                start_angle = e.start*100
                end_angle = e.end*100
                Resolution = float(self.Resolution.text())*100
                angle_set = int(abs(end_angle-start_angle)/Resolution)
                limit += angle_set
